...
class Agent:
    ...
    def plan(self) -> list:
        ...
        # 仅支持LLM版本（模板版本已移除）
        # 如果主角未配置LLM，这里会抛出明确的错误
        self.current_actions = self.planner.generate_daily_plan(self, world_state)
    ...
    def write_diary(self, use_llm=None, llm_client=None) -> str:
        """生成日记（主角强制LLM版本）"""
        from .diary import DiaryWriter

        # 优先使用传入的llm_client，其次使用planner上的llm_client
        effective_llm_client = llm_client
        if not effective_llm_client and hasattr(self.planner, 'llm_client'):
            effective_llm_client = self.planner.llm_client

        diary_writer = DiaryWriter(effective_llm_client)
